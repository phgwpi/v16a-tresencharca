var image1=new Image(); image1.src="ex.gif";
	...
	switch(num){
	case 1: content='¡Has empatado con Chungoboss!';
	break;
	case 2: content='Chungoboss gana esta ronda.';
	break;
	case 3: content='Tú ganas esta ronda.';
	break;
	case 4: content='Nivel de dificultad '+level+'\n\nChungoboss gana: '+pcwins[level]+'  (Total: '+(pcwins[0]+pcwins[1]+pcwins[2]+pcwins[3])+')\nTú ganas: '+playerwins[level]+'  (Total: '+(playerwins[0]+playerwins[1]+playerwins[2]+playerwins[3])+')\nEmpates : '+draws[level]+'  (Total: '+(draws[0]+draws[1]+draws[2]+draws[3])+')';
	break;
	}
	
	document.scores.scores2.value=content
	
	if(num<4)setTimeout('init(4)',3000);
	}
	...
	findwinner(true);
	}else alert('&iexcl;No se puede mover aqu&iacute;!');
	}}
